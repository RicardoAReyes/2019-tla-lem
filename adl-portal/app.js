...
app.use("*", keycloakAdapter.protect(), async function(req, res, next) {
    ...
    // First, get Person data for this user from the LP.
    let personRes = await axios.get(config.lp + `/person?secret=${config.secret}&uuid=${res.locals.user.id}`);
    let userData = personRes.data.find(x => x !== undefined);
    ...
    let authRes = await axios.get(config.authHelper + `?secret=${config.secret}`);
    ...
    console.log("Local storage for " + res.locals.actor.id + ":", localStorage.getItem(res.locals.actor.id));

    // Get the actors's completed tasks and relevant events.
    res.locals.acknowledged = localStorage.getItem(res.locals.actor.id);
    ...
    if (res.locals.roles.includes("Admin") || res.locals.roles.includes("OICS"))
    {
        ...
        // let intGrpRes = await axios.get(config.lp + `/interestGroup?secret=${config.secret}`);
        // let intGrp = intGrpRes.data;

        let intGrpMemberRes = await axios.get(config.lp + `/interestGroupMembers?secret=${config.secret}`);
        ...
        {
            // Find the first interest group in the list we belong to.
            let mainIntGrp = intGrpMembers.find(x => x.memberId === userData.id);
            ...
            res.locals.group = mainIntGrp.groupId;

            for(const t of tlaUsers)
            {
                let thisUserRes = await axios.get(config.lp + `/person?secret=${config.secret}&uuid=${t.id}`);
                let thisUser = thisUserRes.data.find(x => x !== undefined);
                if (thisUser)
                {
                    if (thisUser.uuid !== res.locals.user.id && intGrpMembers.some(x => x.memberId === thisUser.id && x.groupId === res.locals.group))
                    {
                        let student = { id: t.id, name: t.username };
...
app.get(config.root + "/acknowledge", function(req, res) {

    let ackUser = req.query.user;
    let ackTask = req.query.task;
    let ackList = localStorage.getItem(ackUser);

    if (ackList)
    {
        let newList = JSON.parse(ackList);
        if (!newList.tasks.includes(ackTask))
        {
            newList.tasks.push(ackTask);
            localStorage.setItem(ackUser, JSON.stringify(newList));
        }
    }
    else
    {
...
app.get(config.root + "/student/:student", function(req, res) {

    targetStudents[res.locals.user.id] = req.params.student;
    res.redirect(config.root);
});

app.get(config.root + "/view/:view", function(req, res) {
...
async function getEventList(actor) {
    ...
    let verbFilter = ["abandoned", "captured", "completed", "deselected", "initialized", "planned"];
